...
switch (platform) {
  ...
  case 'linux':
    switch (arch) {
      ...
      case 'arm':
        if (isMusl()) {
          localFileExisted = existsSync(join(__dirname, 'fepro-addon.linux-arm-musleabihf.node'))
          try {
            if (localFileExisted) {
              nativeBinding = require('./fepro-addon.linux-arm-musleabihf.node')
            } else {
              nativeBinding = require('windows-api-rs-demo-linux-arm-musleabihf')
            }
          } catch (e) {
            loadError = e
          }
